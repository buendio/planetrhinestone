      ...
      _onSelectChange: function() {
        ...
        if (!variant) {
          return;
              ...
              var pid = this.originalSelectorId.replace(/[^0-9]/g, '');
        ...
        if (cart.item_count === 0) {
          ...
          $.each(cart.items, function(index, product) {
            ...
            $.each(wcpcartdata.items, function( index, wcp_val ) {   
              if(wcp_val.variant_id == product.variant_id){          
                product.price = wcp_val.wcp_v_price;  
                product.compare_at_price = wcp_val.wcp_v_compare_price;                 
                return ;
              }
            });   	
  ...
  theme.predictiveSearch = (function() {
    ...
    function buildProducts(results) {
      ...
      for (var i = results.length - 1; i >= 0; i--) {
        ...
        $.ajax({
          url: '/search?q=id:'+product.id+'&type=product&view=wcp_recent',
          async: false,
          success: function (wpdprd) {
            wpdprd = JSON.parse(wpdprd);
            product.price = wpdprd.wpd[0].ProductMinPrice;
            product.compare_at_price_max = wpdprd.wpd[0].ProductMaxPrice;
          }
        });
        var new_product = {
          title: product.title,
           ...
           var wcpcartdata=[];
            ...
            $.ajax({
                ...
                success: function (wpdprd) {
                  	wpdprd = JSON.parse(wpdprd);
                  	product.price = wpdprd.wpd[0].ProductMinPrice;  
                	product.compare_at_price = wpdprd.wpd[0].ProductMaxPrice;  
                  	product.price_min = wpdprd.wpd[0].ProductMinPrice;  
